    ...
    <div className="sm:p-2 h-full w-full md:flex gap-2 relative bg-gray-50 dark:bg-gray-900 dark:text-white">
      {!open && (
        <button
          ...
          className="md:hidden absolute top-0 right-0 flex items-center gap-2 px-4 py-2 m-2 bg-pink-100 text-pink-600 font-semibold rounded-full shadow"
        >
          <FiFilter /> Filter
        </button>
      )}
      ...
      <div className="flex-1 pt-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 sm:gap-2 gap-1">
          {filtered.map((product) => (
            <Link
              key={product.id}
              to={`/products/${product.id}`}
              className="group bg-white dark:bg-gray-800 border sm:rounded-md p-4 flex flex-col transition-shadow hover:shadow-lg"
            >
